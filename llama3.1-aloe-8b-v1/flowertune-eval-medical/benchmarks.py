    ...
    def post_process(row):
        ...
        for opt in row["options"]:
            if opt["value"] == row["answer"]:
                row["gold"] = opt["key"]
                break
        row["prompt"] = f"{instruction}\n{row['prompt']}\nIMPORTANT: Output ONLY one character. Do NOT write anything else.\n Answer:\n"
        return row
